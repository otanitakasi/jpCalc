...
import { FlatList, StyleSheet, Text, TextInput, Alert, View, Modal, Linking, TouchableHighlight, Button, ScrollView, } from 'react-native';
import { powdersList } from '../drugs_data/powders_list.js';

class Powders extends React.Component {
  ...
  setDrugPickerValue(newValue) {
    ...
    this.setDoseRange(this.state.bodyWeight, this.state.ages, doseRange);
  ...
  setBodyWeight(bodyWeight) {
    ...
    this.setDoseRange(bodyWeight, this.state.ages, this.state.doseRange);
  }
  ...
  setAges(ages) {
    ...
    this.setDoseRange(this.state.bodyWeight, ages, this.state.doseRange);
  }

  //
  // 投与量の上限、下限を設定する
  //
  setDoseRange(bodyWeight, ages, doseRange) {
    let minmax;
    let weight_str = /mg\/kg.*/;

    // 年齢で投与量が決まる
    doseRange.forEach((value, index) => {
      const elm = value.split(/:/);

      if (Number(elm[0]) <= ages) {
        if (weight_str.test(elm[1])) {
        // 体重で投与量が決まる
          minmax = elm[1].replace(/mg\/kg.*/, '').split(/[-]/);
          this.setState({
  ...
  render() {
    ...
    mass =  (isNaN(mass)) ? 0 : (isFinite(mass)) ? mass : 0;

    return (
      <View style={styles.container}>
        {/* 医薬品選択ボタン */}
        <Button onPress={() => this.toggleDrugPicker()} title='医薬品を選択' />
        <View style={{ width: 300, height: 30, backgroundColor: '#3bbfe2',
          alignItems: 'center',
        ...
        >
          ...
          >
            ...
            <View style={{ backgroundColor: '#fff', alignItems: 'center' }}>
              <ScrollView>
                { powdersList.list.map((value, index) => {
                  let pattern;
